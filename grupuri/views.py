...
class BoardsPageView(ListView):
    """
        Implementare ListView de bază pentru a apela lista de grupuri.
        """
    model = Board
...
class BoardPageView(ListView):
        """
        Implementare de bază ListView pentru a apela lista de subiecte pentru fiecare grupa.
        """
    model = Subject
    paginate_by = 20
...
class UserSubscriptionListView(LoginRequiredMixin, ListView):
    """
        Implementare ListView de bază pentru a apela lista de abonamente pentru fiecare utilizator.
        """
    model = Board
    paginate_by = 10
...
def subscribe(request, board):
    """
        Abonează un grup și returnează numărul de abonați.
    """
...
class UserCreatedBoardsPageView(LoginRequiredMixin, ListView):
    """
        Implementare de bază ListView pentru a apela lista de grupuri pentru fiecare utilizator.
        """
    model = Board
    paginate_by = 20
...
def new_board(request):
    """
        Afișează un formular și gestionează acțiunea de creare a unui nou forum.
    """
...
def edit_board_cover(request, board):
    """
    Afișează formularul de editare pentru coperta plăcii și gestionează acțiunea de editare.
        """
    board = get_object_or_404(Board, slug=board)
    if request.method == 'POST':
...
def banned_users(request, board):
    """
        Afișează administratorilor forumului o listă de utilizatori interzisi.
    """
    board = get_object_or_404(Board, slug=board)
